...
#yiru
def read_old_CV_UV_CP(output_path,CV_list,UV_list,CP_list):
        tmp_cv = {}
        tmp_uv = {}
        tmp_cp = {}
        path_num = 0
        # Read in exsiting paths
        record_path = os.path.join(output_path,"CV_UV_CP")
        if os.path.exists(record_path):
            f = open(record_path)
            iter_f = iter(f)
            flag = 0; #0:the first time to read, 2:cv,3: cp,4:uv
            for line in iter_f:
                if line.find("path") > -1 and flag == 0:
                    # generate a new path
                    flag = 1
                    continue
                elif line.find("path") > -1 and flag > 0:
                    # add to CV UV CP list
                    if len(tmp_cv) > 0:
                        CV_list.append(copy.deepcopy(tmp_cv))
                        UV_list.append(copy.deepcopy(tmp_uv))
                        CP_list.append(copy.deepcopy(tmp_cp))
                        path_num = path_num + 1
                    tmp_cv.clear()
                    tmp_uv.clear()
                    tmp_cp.clear()
                    flag = 1
                    continue
                elif line.find("cri_value") > -1:
                    flag = 2
                    continue
                elif line.find("cri_pos") > -1:
                    flag = 3
                    continue
                elif line.find("uncertan_value") > -1:
                    flag = 4
                    continue
                elif flag == 2:
        ...
        return CV_list, UV_list, CP_list, path_num
def add_new_pos(new_cv, new_cp, new_uv, CV_list, UV_list, CP_list,path_num):
    tmp_i = 0
    add_flag = 0
    while tmp_i < path_num:   
        similar_count = 0
        #if new_cv is a subset of CV_list or there are same pos with different value, we will not merge
        for key in CV_list[tmp_i].keys():
            if key in new_cv.keys():
                if CV_list[tmp_i][key] == new_cv[key]:
                    similar_count = similar_count + 1
        if similar_count == len(CV_list[tmp_i]) or similar_count == len(new_cv):
            add_flag = 1
        tmp_i = tmp_i + 1
        
    if add_flag == 0:
        CV_list.append(copy.deepcopy(new_cv))
        UV_list.append(copy.deepcopy(new_uv))
        CP_list.append(copy.deepcopy(new_cp))
        path_num = path_num + 1
    return CV_list, UV_list, CP_list, path_num
...
class SimSolver(SimStatePlugin):
    ...
    def eval(self, e, cast_to: Optional[CastTarget] = None, **kwargs):
        ...
        #yiru
        file_path_pos = self.state.project.filename.rfind("/")
        output_path = self.state.project.filename[:file_path_pos+1]
        #print(output_path)
        ...
        CP_list = []
        path_num = 0
        CV_list, UV_list, CP_list, path_num = read_old_CV_UV_CP(output_path,CV_list,UV_list,CP_list)
        ...
        buf = self.eval_upto(e, 1, cast_to = cast_to,  **kwargs)[0].decode('utf-8',errors='ignore')
        #print(buf)
        for key in CV.keys():
            CV[key] = buf[key]
        ...
        new_cp = {}
        new_uv = {}
        CV_list, UV_list, CP_list, path_num= add_new_pos(CV, new_cp, new_uv, CV_list, UV_list, CP_list,path_num)
        
        #yiru
        write_i = 0
        CV_UV_CP_path = os.path.join(output_path,"CV_UV_CP")
        with open(CV_UV_CP_path, mode = 'w') as fo:
            while write_i < path_num:
                if len(CV_list[write_i]) == 0:
                    write_i = write_i + 1
                    continue
                str1 = "path"+str(write_i) + ":\ncri_value:\n"
                fo.write(str1)
                for key in sorted(CV_list[write_i].keys()):
                    string = str(key) + ":" + str(CV_list[write_i][key]) + "\n"
                    fo.write(string)
                str2 = "cri_pos:\n"
                fo.write(str2)
                if len(CP_list[write_i]) > 0:
                    for key in sorted(CP_list[write_i].keys()):
                        string = str(key) + ":" + str(CP_list[write_i][key]) + "\n"
                        fo.write(string)
                str3 = "uncertan_value:\n"
                fo.write(str3)
                if len(UV_list[write_i]) > 0:
                    for key in sorted(UV_list[write_i].keys()):
                        string = str(key)+ ":" + str(UV_list[write_i][key]) + "\n"
                        fo.write(string)
                write_i = write_i + 1
        #yiru
